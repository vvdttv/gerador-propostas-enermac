...
export interface ResultadoFinanciamento {
  ...
  valorParcelaCarencia: number;    // Parcela por período (mensal/trimestral/semestral/anual)
  valorParcelaAmortizacao: number; // Parcela por período (mensal/trimestral/semestral/anual)
  ...
  frequenciaParcela: 'mensal' | 'trimestral' | 'semestral' | 'anual';
  parcelasPorAno: number;       // Quantas parcelas por ano
}

/**
 * Calcula o financiamento com juros compostos
 * - Suporta parcelas mensais, trimestrais, semestrais ou anuais
...
export function calcularFinanciamento(
  ...
  frequenciaParcela: 'mensal' | 'trimestral' | 'semestral' | 'anual' = 'anual'
  ...
  // Parcelas por ano conforme frequência
  const parcelasPorAno: Record<string, number> = {
    mensal: 12,
    trimestral: 4,
    semestral: 2,
    anual: 1,
  ...
  const parcelasAno = parcelasPorAno[frequenciaParcela];
  
  // Taxa por período de parcela
  const taxaPeriodoParcela = Math.pow(1 + taxaMensal, 12 / parcelasAno) - 1;
  
  // Número total de parcelas
  const parcelasCarencia = carenciaAnos * parcelasAno;
  const parcelasAmortizacao = (prazoAnos - carenciaAnos) * parcelasAno;
  
  // Durante carência: juros sobre o principal financiado (por período de parcela)
  const jurosPeriodoCarencia = valorPrincipal * taxaPeriodoParcela;
  const valorParcelaCarencia = jurosPeriodoCarencia;
  const totalJurosCarencia = valorParcelaCarencia * parcelasCarencia;
  
  let valorParcelaAmortizacao = 0;
  let totalJurosAmortizacao = 0;
  
  if (parcelasAmortizacao > 0) {
    // Fórmula Price para parcelas por período
    valorParcelaAmortizacao = valorPrincipal * 
      (taxaPeriodoParcela * Math.pow(1 + taxaPeriodoParcela, parcelasAmortizacao)) / 
      (Math.pow(1 + taxaPeriodoParcela, parcelasAmortizacao) - 1);
 ...
 * Regra de diárias: até 250km = 0, 251-650km = 1, acima de 650km = 2, a cada 650km = +2
 */
export function calcularManutencao(
  params: ParametrosManutencao,
  horasFuncionamentoGeradorDia: number
): ResultadoManutencao {
  // Regra de hospedagem por distância - NOVA LÓGICA
  // Até 250km = 0 diárias
  // 251-650km = 1 diária
  // Acima de 650km = 2 diárias + 2 diárias para cada 650km adicional
  let qtdDiarias = 0;
  ...
  if (params.distanciaKm <= 250) {
    ...
    // Acima de 650 km → 2 diárias + 2 para cada 650km adicional
    const kmAdicional = params.distanciaKm - 650;
    const blocosAdicionais = Math.floor(kmAdicional / 650);
    qtdDiarias = 2 + (blocosAdicionais * 2);
    qtdAlimentacao = qtdDiarias;
  }
...
export interface CenarioViabilidade {
  ...
  resultadoLiquidoMedioMensal: number;  // (resultadoLiquido20Anos / 20) / 12
...
function calcularCenarios(
  ...
  const resultados: CenarioViabilidade[] = cenarios.map(cenario => {
    ...
    return {
      ...
      resultadoLiquidoMedioMensal: ((receitaBrutaDisponivelAno * 20) - financiamento.valorTotalPago - (custoTotalAno * 20)) / 20 / 12,
      // Indicadores de Retorno
      anosPontoEquilibrio,
      anosPayback,
      parcelaCarenciaAno,
  ...
  const mediaCenario: CenarioViabilidade = {
    ...
    resultadoLiquidoMedioMensal: ((receitaBrutaDisponivelAnoMedia * 20) - financiamento.valorTotalPago - (custoTotalAno * 20)) / 20 / 12,
    // Indicadores de Retorno
...
// ===== FLUXO DE CAIXA ANUAL (10 ANOS) =====

// Interface para fluxo de caixa de um ano
export interface FluxoCaixaAnualItem {
  ano: number;
  fase: 'entrada' | 'carencia' | 'amortizacao' | 'pos_financiamento';
  
  // Receitas (Ganhos)
  receitaVendaEnergia: number;      // Receita com venda de energia
  economiaEnergia: number;          // Economia com energia própria
  receitaTotal: number;             // Total de receitas
  
  // Despesas (Gastos)
  parcelaFinanciamento: number;     // Parcela do financiamento
  custoManutencao: number;          // Custo com manutenção
  despesaTotal: number;             // Total de despesas
  
  // Resultado
  resultadoAno: number;             // Resultado do ano
  acumulado: number;                // Resultado acumulado
}

// Interface para fluxo de caixa completo (10 anos)
export interface FluxoCaixa10Anos {
  anos: FluxoCaixaAnualItem[];
  totalReceitas: number;
  totalDespesas: number;
  resultadoFinal: number;
  taxaTarifa: number;               // Tarifa utilizada para o cálculo
}

/**
 * Calcula o fluxo de caixa anual por 10 anos
 * Mostra quanto foi gasto, quanto foi ganho e qual foi o resultado
 */
export function calcularFluxoCaixa10Anos(
  kwhAno: number,
  gastoMensalEnergia: number,
  tarifaVendaKwh: number,
  financiamento: ResultadoFinanciamento,
  ...
  const anos: FluxoCaixaAnualItem[] = [];
  
  // Receitas anuais
  const receitaVendaEnergiaAno = kwhAno * tarifaVendaKwh;
  const economiaEnergiaAno = gastoMensalEnergia * 12;
  const receitaTotalAno = receitaVendaEnergiaAno + economiaEnergiaAno;
  
  // Despesas anuais (manutenção)
  const custoManutencaoAno = custoManutencaoMensal * 12;
  
  // Parcelas ANUAIS do financiamento (cada parcela é por período, multiplicar para ter anual)
  const parcelaCarenciaAnual = financiamento.valorParcelaCarencia * financiamento.parcelasPorAno;
  const parcelaAmortizacaoAnual = financiamento.valorParcelaAmortizacao * financiamento.parcelasPorAno;
  
  let acumulado = -valorEntrada; // Entrada é o investimento inicial
  
  for (let ano = 1; ano <= 10; ano++) {
    let fase: 'entrada' | 'carencia' | 'amortizacao' | 'pos_financiamento';
    let parcelaFinanciamento: number;
    
    if (ano <= financiamento.carenciaAnos) {
      // Período de carência
      fase = 'carencia';
      parcelaFinanciamento = parcelaCarenciaAnual;
    } else if (ano <= financiamento.prazoAnos) {
      // Período de amortização
      fase = 'amortizacao';
      parcelaFinanciamento = parcelaAmortizacaoAnual;
    } else {
      // Pós-financiamento
      fase = 'pos_financiamento';
      parcelaFinanciamento = 0;
    }
    
    const despesaTotal = parcelaFinanciamento + custoManutencaoAno;
    const resultadoAno = receitaTotalAno - despesaTotal;
    acumulado += resultadoAno;
    
    anos.push({
      ano,
      fase,
      receitaVendaEnergia: receitaVendaEnergiaAno,
      economiaEnergia: economiaEnergiaAno,
      receitaTotal: receitaTotalAno,
  ...
  // Totais
  const totalReceitas = anos.reduce((sum, a) => sum + a.receitaTotal, 0);
  const totalDespesas = anos.reduce((sum, a) => sum + a.despesaTotal, 0) + valorEntrada;
  const resultadoFinal = anos[anos.length - 1].acumulado;
